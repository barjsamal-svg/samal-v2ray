...
class MainActivity : HelperBaseComponentActivity() {
        ...
        MainScreen(
            ...
            onAction = { action ->
                when (action) {
                    ...
                    is MainAction.ShareSamal -> shareSamalAsync(action.guid, action.expireDate, action.message)
    ...
    private fun shareSamalAsync(guid: String, expireDate: String, message: String) {
        lifecycleScope.launch(Dispatchers.IO) {
            val result = AngConfigManager.shareSamalConfig(this@MainActivity, guid, expireDate, message)
            withContext(Dispatchers.Main) {
                if (result == 0) toastSuccess(R.string.toast_success)
                else toastError(R.string.toast_failure)
            }
