...
object AngConfigManager {
    ...
    /**
     * Shares the configuration as a SAMAL encrypted string.
     */
    fun shareSamalConfig(context: Context, guid: String, expireDate: String, message: String): Int {
        try {
            val conf = shareConfig(guid)
            if (TextUtils.isEmpty(conf)) return -1
            
            val encrypted = SamalCrypto.encryptConfig(conf, expireDate, message)
            if (encrypted.isEmpty()) return -1
            
            Utils.setClipboard(context, encrypted)
            return 0
        } catch (e: Exception) {
            LogUtil.e(AppConfig.TAG, "Failed to share SAMAL config", e)
            return -1
        }
    }
    ...
    fun importBatchConfig(server: String?, subid: String, append: Boolean): Pair<Int, Int> {
        ...
        var samalRes: SamalResult? = null
        if (server?.startsWith("SAMAL:") == true) {
            samalRes = SamalCrypto.decryptConfig(server)
            if (samalRes != null) {
                finalServer = samalRes.data
            }
        }
        var count = parseBatchConfig(Utils.decode(finalServer), subid, append)
        
        // If it was a SAMAL config, we need to find the imported GUID and save metadata
        if (samalRes != null && count > 0) {
            // This is a bit tricky as parseBatchConfig saves multiple. 
            // For a single SAMAL file, it's usually one.
            val selected = MmkvManager.getSelectServer()
            if (selected != null) {
                MmkvManager.encodeSamalMetadata(selected, samalRes.customMessage, samalRes.expireDate)
            }
        }
        if (count <= 0) {
